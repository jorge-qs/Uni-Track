...
def run_benchmark(users=10, spawn_rate=2, duration="30s", host="http://localhost:8000"):
    ...
    print(f"--- Starting Benchmark (Results: {results_dir}) ---")
    
    # 1. Start Resource Monitor in background
    print("Starting Resource Monitor...")
    monitor_cmd = [sys.executable, monitor_script, "--process", "uvicorn", "--output", os.path.join(results_dir, "system_metrics.csv")]
    ...
    locust_csv_prefix = os.path.join(results_dir, "locust_stats")
...
def analyze_results(metrics_dir):
    ...
    print("="*30 + "\n")
    print(f"Full results saved in: {metrics_dir}")
